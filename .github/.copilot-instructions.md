...
GNOME Wardrive is a modern WiFi wardriving application built for the GNOME desktop using Python, GTK4, and Libadwaita. The application is designed to be **completely portable** and work seamlessly with native system integration without requiring any system configuration changes.
...
- Direct system integration for optimal performance
...
# Artifact name: gnome-wardrive-deb-{run_number}-{commit_sha}
...
1. **Zero System Configuration**: Application must work immediately after package installation
2. **Native Integration**: Full functionality with direct system access
...
```yaml
D-Bus Services:
  - org.freedesktop.NetworkManager  # WiFi monitoring  
  - org.freedesktop.GeoClue2        # GPS location
  - org.freedesktop.FileManager1    # File operations
...
**❌ NEVER require these system changes** (breaks portability):
- Root privileges or sudo access
- Custom PolicyKit rules
...
1. **No Permission Errors**: Application starts clean with system access
2. **WiFi Detection Works**: Networks discovered through NetworkManager
3. **Location Services Work**: GPS coordinates obtained via GeoClue D-Bus
4. **Data Export Works**: Files can be saved to user directories
...
### 4. Package Testing
- **Problem**: Only testing development builds
- **Solution**: Always test packaged builds for real-world compatibility
...
- **Meson Build System**: 0.60+ (for modern GNOME integration)
